...
class ProjectionConversionRetryTests(unittest.TestCase):
    ...
    @patch("config.settings.get_settings", side_effect=AttributeError("missing settings field"))
    def test_conversion_output_profile_falls_back_when_settings_invalid(self, _mock_get_settings):
        profile = get_conversion_output_profile()
        self.assertEqual(profile["target_width"], 4320)
        self.assertEqual(profile["target_height"], 2160)
        self.assertEqual(profile["crf"], 16)
    ...
    @patch("config.settings.get_settings")
    def test_conversion_output_profile_clamps_negative_crf(self, mock_get_settings):
        mock_get_settings.return_value = types.SimpleNamespace(
            conversion_target_width=4320,
            conversion_target_height=2160,
            conversion_crf=-999,
            conversion_preset="medium",
        )
        profile = get_conversion_output_profile()
        self.assertEqual(profile["crf"], 0)
